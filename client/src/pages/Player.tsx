...
const Player = () => {
  ...
  useEffect(() => {
    const loadAdScript = (containerId: string, key: string, width: number, height: number) => {
      try {
        ...
        // Set atOptions globally for this specific ad with error handling
        try {
          ...
          console.warn('Error setting atOptions:', e);
        }

        // Create and load the ad script with enhanced error handling
        ...
        script.onload = () => {
          console.log(`Ad script loaded successfully for ${containerId}`);
        };
        
        script.onerror = (error) => {
          console.warn(`Ad script failed to load for ${containerId}:`, error);
          try {
            adWrapper.innerHTML = '<div style="color: #666; text-align: center; padding: 20px; font-size: 14px;">Advertisement</div>';
          } catch (e) {
            console.warn('Error setting fallback content:', e);
          }
        };
        ...
        try {
          ...
          console.warn('Error appending script to head:', e);
          adWrapper.innerHTML = '<div style="color: #666; text-align: center; padding: 20px; font-size: 14px;">Advertisement</div>';
          return;
        }
        ...
        setTimeout(() => {
          try {
            ...
            console.warn('Error in timeout fallback:', e);
          }
        }, 5000);

      } catch (error) {
        console.warn(`Error in loadAdScript for ${containerId}:`, error);
      }
    };

    const loadProfitableRateAd = () => {
      try {
        const container = document.getElementById('container-51b35925a8ed6839e3d27a6668f25975');
        if (!container) {
          console.warn('Profitable Rate CPM container not found');
          return;
        }
        ...
        // Check if script already exists and remove it
        const existingScript = document.querySelector('script[src*="pl27267578.profitableratecpm.com"]');
        if (existingScript) {
          existingScript.remove();
        }
        ...
        script.onload = () => {
          console.log('Profitable Rate CPM ad script loaded successfully');
        };
        
        script.onerror = (error) => {
          console.warn('Profitable Rate CPM ad script failed to load:', error);
          try {
            container.innerHTML = '<div style="color: #666; text-align: center; padding: 20px; font-size: 14px;">Advertisement</div>';
          } catch (e) {
            console.warn('Error setting fallback content for profitable rate ad:', e);
          }
        };
        ...
        try {
          ...
          console.warn('Error appending profitable rate script to head:', e);
          container.innerHTML = '<div style="color: #666; text-align: center; padding: 20px; font-size: 14px;">Advertisement</div>';
          return;
        }
        ...
        setTimeout(() => {
          try {
            ...
            console.warn('Error in profitable rate timeout fallback:', e);
          }
        }, 5000);

      } catch (error) {
        console.warn('Error in loadProfitableRateAd:', error);
      }
    };

    // Load ads with increased delay to ensure DOM is fully ready
    const timer = setTimeout(() => {
      try {
        console.log('Starting ad loading process for Player page');
        loadProfitableRateAd();
        loadAdScript('player-ad-2', '9b4e8b40e2674e797095c9825bbf58f0', 728, 90);
        loadAdScript('player-ad-3', 'bae2d0d07c2b8e24730cc20afdd2faa5', 300, 250);
        loadAdScript('player-ad-4', '78e88b204108820f7e99c412c9223d3f', 160, 300);
      } catch (error) {
        console.warn('Error in ad loading timer:', error);
      }
            ...
            <div className="w-full advertisement-section" data-auto-click-ad>
              <Card className="bg-gradient-to-br from-black/40 via-[#0A7D4B]/10 to-black/40 backdrop-blur-sm border border-border/30">
                <CardContent className="p-4 sm:p-6 space-y-6 sm:space-y-8">
                  {/* First Ad - Profitable Rate CPM */}
                  <div className="w-full flex justify-center">
                    <div className="w-full max-w-4xl min-h-[120px] bg-gray-900/20 rounded-lg p-4 flex items-center justify-center">
                      <div id="player-ad-1" className="w-full h-full flex items-center justify-center">
                        <div id="container-51b35925a8ed6839e3d27a6668f25975" style={{minHeight: '90px', width: '100%', textAlign: 'center'}}></div>
                      </div>
                    </div>
                  </div>

                  {/* Second Ad - 728x90 Banner */}
                  <div className="w-full flex justify-center">
                    <div className="w-full max-w-3xl min-h-[110px] bg-gray-900/20 rounded-lg p-4 flex items-center justify-center">
                      <div id="player-ad-2" className="w-full h-full flex items-center justify-center"></div>
                    </div>
                  </div>

                  {/* Third and Fourth Ads - Side by Side on Desktop, Stacked on Mobile */}
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8">
                    {/* Third Ad - 300x250 Medium Rectangle */}
                    <div className="flex justify-center">
                      <div className="w-full max-w-sm min-h-[270px] bg-gray-900/20 rounded-lg p-4 flex items-center justify-center">
                        <div id="player-ad-3" className="w-full h-full flex items-center justify-center"></div>
                      </div>
                    </div>

                    {/* Fourth Ad - 160x300 Wide Skyscraper */}
                    <div className="flex justify-center">
                      <div className="w-full max-w-xs min-h-[320px] bg-gray-900/20 rounded-lg p-4 flex items-center justify-center">
                        <div id="player-ad-4" className="w-full h-full flex items-center justify-center"></div>
                      </div>
                    </div>
                  </div>
                </CardContent>
