...
const AutoClickAd = () => {
  useEffect(() => {
    const clickRandomAd = () => {
      ...
      setTimeout(() => {
        // Only look for ad containers within specific ad sections to prevent clicking across entire page
        const adSectionSelectors = [
        ...
        let adSectionElements = [];
        adSectionSelectors.forEach(selector => {
          try {
            const elements = document.querySelectorAll(selector);
            adSectionElements.push(...Array.from(elements));
          } catch (e) {
            // Ignore invalid selectors
          }
        });
        
        if (adSectionElements.length === 0) {
          console.log('No ad sections found for auto-click');
          return;
        }
        
        // Look for ad containers within ad sections only
        const adSelectors = [
          '[data-ad]',
          '.ad',
          '.ads',
        ...
        // Find ad elements only within ad sections
        adSectionElements.forEach(adSection => {
          adSelectors.forEach(selector => {
            try {
              const elements = adSection.querySelectorAll(selector);
